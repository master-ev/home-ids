...
CONTEXT_FEATURES = ["ctx_src_flows", "ctx_src_ports", "ctx_src_dsts", "ctx_flows_per_port", "ctx_dst_sources",]
...
def compute_context(flow_list):
    ...
    for packets in flow_list:
        ...
        if source_key not in per_source:
            per_source[source_key] = {"flows": 0, "ports": set(), "destinations": set()}
    ...
    for source, destination, window in described:
        ...
        context = {"ctx_src_flows": flow_count, "ctx_src_ports": port_count, "ctx_src_dsts": len(source_stats["destinations"]), "ctx_flows_per_port": flows_per_port, "ctx_dst_sources": len(destination_sources),}
