...
Generated 2026-09-26 02:28 at commit `2ecd610` (+ uncommitted changes) by `metrics_report.py`.
...
- **Attack alerts**: 188 logged, 45 notified (cooldown 60 s per source/destination/family)
...
| syn_flood1.pcap | syn_flood | yes | yes | yes | yes | - | 8 | 1 | syn_flood x1 | syn_flood x8 |
| syn_flood2.pcap | syn_flood | yes | yes | yes | yes | - | 8 | 1 | syn_flood x1 | syn_flood x8 |
...
scan.pcap               scan           100.0   100.0   100.0   100.0
...
mean                                    93.8    99.1    99.0    97.7
worst                                    0.5    88.6    88.6    74.4
Results below 80%: what did the model say?
set A  syn_flood1.pcap          true=syn_flood   predicted={'udp_scan': 210, 'syn_flood': 1}
set D  dns_div3.pcap            true=normal      predicted={'normal': 90, 'dos': 31}
```

## Live validation

Point-in-time measurements on a real interface, **not regenerated** by this report.

- Measured: 2026-09-25 on eth1
- Generator: hping3 --flood from WSL to router 192.168.1.1

### Capture-path loss (tcpdump, kernel counters)

- **port_scan_full**: 11236 received, 0 dropped (0.0% kernel loss)
- **syn_flood_plain**: 200237 received, 0 dropped (0.0% kernel loss)

### Live detection

- **port_scan_nmap_sS**: detected (trackers + model), 250 ports
- **syn_flood_payload**: detected (tracker only (model unsure)), 7411 half-open
- **syn_flood_plain**: detected (model + tracker), 13269 half-open

_Point-in-time lab measurements, not regenerated by metrics_report. Source is local WSL, single machine; a payload flood and a full-connection flood were not stress-tested for drops._
